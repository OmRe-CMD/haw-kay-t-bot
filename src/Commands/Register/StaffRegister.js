...
if(err) return message.channel.send(new MessageEmbed().setDescription(`Kullanıcının statı getirilirken bir sorun ile karşı karşıya kaldık.`).setAuthor(member.user.tag, member.user.avatarURL({dynamic:true})).setColor(env.Colors.red).setFooter(`Developed By ÖMRE.`)).then(x => x.delete({timeout: 6500}))
if(data.length < 1) { message.channel.send(new MessageEmbed().setDescription(`Kullanıcı hiç erkek/kadın kayıtı yapmamış bundan dolayı veriler aktarılamıyor.`).setAuthor(member.user.tag, member.user.avatarURL({dynamic:true})).setColor(env.Colors.red).setFooter(`Developed By ÖMRE.`)).then(x => x.delete({timeout: 6500}))}
else { 
let StrgBaslik = 'Yetkilinin Kayıtları.' 
message.channel.send(new MessageEmbed().setDescription(`**${message.guild}** sunucusunda ${member} adlı yetkili toplam \`${StaffCheck.allRegistry}\` kayıt işlemi gerçekleştirmiş.\n\nYetkilinin daha detaylı kayıt dökümanlarının listesi;\nToplam Kayıtları: \`${StaffCheck.allRegistry}\`\nErkek Kayıtları: \`${StaffCheck.manRegistry}\`\nKadın Kayıtları: \`${StaffCheck.womanRegistry}\``).setAuthor(StrgBaslik, member.user.avatarURL({dynamic:true})).setColor(env.Colors.anlik).setFooter(`Developed By ÖMRE.`))}})};
